...
class StrategyBase(
    ...
    def on_order_response(self, response: dict):
        ...
        if orig_qty == exec_qty:
            ...
            logger.info(
                'Order %s filled qty=%s at t=%d',
                order_id, exec_qty, transact_time)
        elif exec_qty > 0:
            logger.info(
                'Order %s partly filled %s/%s at t=%d. status: %s',
                order_id, exec_qty, orig_qty, transact_time, status)
        else:
            logger.info(
                'Order %s not filled at t=%d. status: %s',
                order_id, transact_time, status)
            return
